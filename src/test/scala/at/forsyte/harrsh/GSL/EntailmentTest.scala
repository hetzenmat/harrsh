...
class EntailmentTest extends AnyFlatSpec {
  ...
  "ab" should "jf" in {
    ...
    val ac = AliasingConstraint(Seq(SortedSet.from(Seq(a, x1)), SortedSet.from(Seq(b, x2)), SortedSet.from(Seq(c))), Map((a, 0), (x1, 0), (x2, 1), (b, 1), (c, 2)))
    ...
    val s = new TypeComputation(lseg, parseFormula("a -> nil")).types(
      AliasingConstraint(Seq(SortedSet.from(Seq(a))), Map((a, 0))))
